...
class WaveNetSpectrogram(nn.Module):
    def __init__(
        ...
        self.in_channels = in_channels - 1 + 4
        self.spec_conv = nn.ModuleList()
        for i in range(self.out_chans):
            tmp_blocks = [
                wave_block(wave_layers[i], self.in_channels, base_filters[0], kernel_size, downsample)
            ]
    ...
    def forward(self, x):  # x: (bs, ch, w)
        x_emb = x[:, 0, :]
        x_emb = self.embed(x_emb.long())
        x = torch.cat([x_emb.transpose(1, 2), x[:, 1:, :]], dim=1)
        out = []
